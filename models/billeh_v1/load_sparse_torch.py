...
def to_torch_sparse(indices, weights, dense_shape, device="cpu", dtype=torch.float32):
    if len(indices) == 0:
        ...
        val = torch.as_tensor(weights, dtype=dtype, device=device)

    # return torch.sparse_coo_tensor(idx, val, size=dense_shape, device=device).coalesce()
...
def load_input(path='/data/allen/v1_model/GLIF_network/input_dat.pkl',
    ...
    for input_population, input_pop_ind in zip(d,range(len(d))):
        ...
        if len(post_indices) == 0:
            ...
            delays = np.concatenate(delays).astype(np.float32)
            indices, weights = sort_indices(indices, weights)
...
def load_billeh(n_input, n_neurons, core_only, data_dir, seed=3000, connected_selection=False, n_output=2,
    ...
    l5e_neuron_sel = np.zeros(network['n_nodes'], bool)
    ...
    if localized_readout:
        ...
        n_needed = neurons_per_output * 15
        replace = l5e_neuron_indices.size < n_needed

        if replace:
            print(
                f"Warning: only {l5e_neuron_indices.size} L5e neurons available; "
                f"sampling {n_needed} readout neurons with replacement"
            )
        ...
        readout_neurons_random = readout_neurons_random.reshape((15, neurons_per_output))
...
def load_billeh_torch(
    ...
    **kwargs,
    ...
    rec = network["synapses"]
    recurrent_sparse = to_torch_sparse(
        rec["indices"],
        rec["weights"],
        rec["dense_shape"],
        device=device,
    ...
    input_sparse = to_torch_sparse(
        input_population["indices"],
        input_population["weights"],
        (4 * network["n_nodes"], input_population["n_inputs"]),
        device=device,
    )
    ...
    torch_data = {
        ...
        "bkg_weights": torch.as_tensor(bkg_weights, dtype=torch.float32, device=device),
        "recurrent_sparse": recurrent_sparse,
        "input_sparse": input_sparse,
        "node_type_ids": torch.as_tensor(network["node_type_ids"], dtype=torch.long, device=device),
    }

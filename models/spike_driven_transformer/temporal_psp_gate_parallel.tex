\section{TemporalPSPGate 的 true-PSP GPU 实现}
...
\subsection{Why the old gate was not true PSP}
...
在 true-PSP 方案下，时序动力学由现有 neuron 路径负责，因此 GPU kernel 不重写 PSP/membrane dynamics。本轮 GPU 化仅下沉下列逐元素运算：
\[
...
\begin{itemize}
  ...
  \item true-PSP gate 的逐元素融合：由 Triton kernel 完成；
...
因此，true-PSP gate 的语义更加严格，也更符合“用真实 PSP 调制 spike attention”的目标。